...
export function renderEnemyView(options: EnemyViewOptions) {
  ...
  if (alive) root.add(renderIntent(scene, context, assets, enemy.intent.type, enemy.intent.amount ?? 0, x + 36, y + 132));
  else root.add(renderStatPill(scene, x + 20, y + 128, "已擊倒", 0x6b7280));
  root.add(
